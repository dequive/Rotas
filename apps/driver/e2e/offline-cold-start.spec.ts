...
test("reabre o PWA offline com bootstrap da mesma identidade", async ({
  ...
  await expect
    .poll(() =>
      offlinePage.evaluate(async () => {
        ...
        return Boolean(await apiCache.match("/api/v1/driver/bootstrap"));
      }),
    )
    .toBe(false);
  ...
  await offlinePage.evaluate(() => {
    ...
    (window as typeof window & { __syncRequests?: typeof captured }).__syncRequests = captured;
    window.fetch = async (input, init) => {
      ...
      if (url.pathname === "/api/v1/sync/batch") {
        const body = JSON.parse(String(init?.body)) as {
          operations: Array<{ local_id: string; idempotency_key: string }>;
        };
  ...
  await expect
    .poll(() =>
      offlinePage.evaluate(
        () =>
          (window as typeof window & { __syncRequests?: Array<unknown> })
