...
describe("Driver Offline-First & Sync Queue", () => {
  beforeEach(async () => {
    ...
    localStorage.setItem("rotas_driver_id", "driver-test");
    localStorage.setItem("rotas_session_id", "session-test");
  ...
  it("renova uma sessão expirada e reutiliza o token no mesmo lote", async () => {
    ...
    const fetchMock = vi.fn().mockImplementation((input: RequestInfo | URL) => {
      ...
      const batchCalls = fetchMock.mock.calls.filter(([candidate]) =>
        String(candidate).includes("/sync/batch"),
      ).length;
      return Promise.resolve(
        batchCalls === 1
          ? new Response(null, { status: 401 })
          : new Response(JSON.stringify(batchSuccess), {
              status: 200,
              headers: { "Content-Type": "application/json" },
            }),
      );
  ...
  it("cache bootstrap só é recuperado para o mesmo tenant e motorista", async () => {
    ...
    expect(
      (
        await getBootstrapCache<typeof snapshot>(
          "tenant-a",
          "driver-a",
          "session-a",
        )
    ...
    expect(
      await getBootstrapCache("tenant-a", "driver-a", "session-b"),
    ).toBeUndefined();
  });
});
