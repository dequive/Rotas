"""Unit and repository tests for the GitHub Actions supply-chain validator."""
...
class ActionPinPolicyTests(unittest.TestCase):
    ...
    def test_rejects_unapproved_owner(self) -> None:
        finding = validate_reference(Path("ci.yml"), f"dequive/tool@{PIN}")
        self.assertIsNotNone(finding)
        assert finding is not None
    ...
    def test_repository_workflows_are_immutable(self) -> None:
        references, findings = evaluate(REPO_ROOT / ".github" / "workflows")
