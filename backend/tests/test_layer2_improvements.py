...
@pytest.mark.asyncio
async def test_trip_patch_and_cache_invalidation(
    ...
    driver = Driver(tenant_id=tenant_id, full_name="Driver Layer2", status="active")
    ...
    try:
        ...
        response = await async_client.patch(
            f"/api/v1/trips/{trip.id}", json=payload, headers=auth_headers
        )
        ...
        payload_exceeded = {"cargo_weight": 6000}
        response = await async_client.patch(
            f"/api/v1/trips/{trip.id}", json=payload_exceeded, headers=auth_headers
        )
        ...
        response = await async_client.patch(
            f"/api/v1/trips/{trip.id}", json=payload_override, headers=auth_headers
        )
        assert response.status_code == 200
...
async def test_trip_order_assignment_payload_capacity(async_client, db, tenant_id, auth_headers):
    ...
    driver = Driver(tenant_id=tenant_id, full_name="Driver Assign", status="active")
    db.add_all([vehicle, driver])
    await db.commit()
    await db.refresh(vehicle)
    await db.refresh(driver)
...
@pytest.mark.asyncio
async def test_cargo_mutations_invalidate_cache(
    async_client, db, tenant_id, auth_headers, mock_redis
):
    ...
    try:
        ...
        mock_redis.delete.reset_mock()
        response = await async_client.post(
            f"/api/v1/trips/{trip.id}/load-permits", json=payload, headers=auth_headers
        )
        assert response.status_code == 200
        mock_redis.delete.assert_any_call(f"tenant:limits:{tenant_id}")
        ...
        mock_redis.delete.reset_mock()
        response = await async_client.post(
            f"/api/v1/trips/{trip.id}/guia-remessa", json=payload_guia, headers=auth_headers
        )
...
@pytest.mark.asyncio
async def test_operational_exceptions_invalidate_cache(
    async_client, db, tenant_id, auth_headers, mock_redis
):
    ...
    try:
        ...
        mock_redis.delete.reset_mock()
        response = await async_client.post(
            f"/api/v1/operational-exceptions/{exc.id}/acknowledge", headers=auth_headers
        )
        assert response.status_code == 200
        mock_redis.delete.assert_any_call(f"tenant:limits:{tenant_id}")
