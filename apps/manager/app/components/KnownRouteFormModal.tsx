    ...
    <>
      ...
      <ModalDialog open={open} onClose={() => setOpen(false)} title={isEdit ? "Editar destino" : "Novo destino"}>
        <form onSubmit={handleSubmit} className="px-6 pb-6 pt-4 flex flex-col gap-3.5">
          ...
          <label className={lbl}>Notas<textarea name="notes" rows={2} defaultValue={route?.notes ?? ""} placeholder="Observações sobre a rota, portagens, etc." className="px-2.5 py-2 border border-border-strong rounded-md bg-surface text-[14px] text-ink w-full focus:outline-none focus:border-focus focus:ring-2 focus:ring-focus-soft resize-y" /></label>
