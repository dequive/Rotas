...
| Transporte e Carga | C2 avançado local; não fechado | listas, requisitos, pedidos, download, checklist Manager canónica, terminal read-only, recovery e E2E verdes; faltam integração e Android |
...
| Transporte e Carga | trip_orders, trips, checklists, cargo, operations | C2 avançado local; não fechado | faltam repetição integrada e Android |
...
| trips | C2 avançado local | criação proibida, listas, detalhe, download, UI, cache tenant+driver+session-scoped e E2E verdes; falta certificação integrada |
