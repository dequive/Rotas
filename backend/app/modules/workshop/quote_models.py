...
class WorkshopQuote(Base):
    ...
    related_work_order_id: Mapped[uuid.UUID | None] = mapped_column(
    ...
    acceptance_channel: Mapped[str | None] = mapped_column(
        String(30), nullable=True
    )  # presencial | whatsapp | telefone | assinatura_digital
...
class WorkshopQuoteItem(Base):
    ...
    quote_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("workshop_quotes.id", ondelete="CASCADE"), index=True
    )
    ...
    part_id: Mapped[uuid.UUID | None] = mapped_column(
        ForeignKey("spare_parts_inventory.id"), nullable=True
    )
