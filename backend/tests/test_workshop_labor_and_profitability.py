...
from app.modules.workshop.service import create_work_order
...
async def test_staff_hourly_rate_historical_effective_from_selection(db, tenant_id, test_user):
    """Selecciona a taxa histórica efectiva na data de conclusão da sessão."""

    # Taxa 1: 400 MZN a partir de 01/01/2026
    await set_staff_hourly_rate(
        db, tenant_id, test_user.id, 400.00, date(2026, 1, 1), actor_id=test_user.id
    )
    # Taxa 2: 600 MZN a partir de 01/06/2026
    await set_staff_hourly_rate(
        db, tenant_id, test_user.id, 600.00, date(2026, 6, 1), actor_id=test_user.id
    )

    # 1. Sessão concluída a 15/05/2026 -> Deve aplicar Taxa 1 (400 MZN)
    rate_may = await get_staff_hourly_rate(
        db, tenant_id, test_user.id, datetime(2026, 5, 15, tzinfo=UTC)
    )
    assert rate_may == Decimal("400.00")

    # 2. Sessão concluída a 05/06/2026 -> Deve aplicar Taxa 2 (600 MZN)
    rate_june = await get_staff_hourly_rate(
        db, tenant_id, test_user.id, datetime(2026, 6, 5, tzinfo=UTC)
    )
...
async def test_multi_mechanic_labor_logging_and_void_exclusion(db, tenant_id, test_user):
    ...
    await set_staff_hourly_rate(db, tenant_id, mec_a_id, 500.00, date(2026, 1, 1))
    ...
    task = WorkOrderTask(
        tenant_id=tenant_id,
        work_order_id=wo["id"],
        description="Diagnóstico e Desmontagem",
        status="in_progress",
    )
    db.add(task)
    await db.flush()
    ...
    voided_log2 = await void_task_labor_session(
        db, tenant_id, log2.id, void_reason="Erro de digitação de minutos", actor_id=test_user.id
    )
...
async def test_post_billing_labor_immutability_guard_409(db, tenant_id, test_user):
    ...
    log = await add_task_labor_session(
        db, tenant_id, task.id, test_user.id, minutes_worked=60, actor_id=test_user.id
    )
    ...
    await close_work_order(
        db, tenant_id, wo["id"], payload=WorkOrderCloseRequest(notes="Done"), actor_id=test_user.id
    )
    ...
    with pytest.raises(ApiError) as exc_add:
        await add_task_labor_session(
            db, tenant_id, task.id, test_user.id, minutes_worked=30, actor_id=test_user.id
        )
    ...
    with pytest.raises(ApiError) as exc_void:
        await void_task_labor_session(
            db, tenant_id, log.id, void_reason="Tentativa pós-fatura", actor_id=test_user.id
        )
...
@pytest.mark.asyncio
async def test_work_order_profitability_with_multi_quote_consolidated_invoice_fk_join(
    db, tenant_id, test_user
):
    """Valida a FK directa numa factura com orçamento original e suplementar."""
    ...
    wo_res = await db.execute(
        select(WorkOrder).where(
            WorkOrder.tenant_id == tenant_id, WorkOrder.vehicle_id == vehicle.id
        )
    )
    ...
    await issue_parts_for_work_order(
        db, tenant_id, wo.id, part.id, Decimal("2.0"), actor_id=test_user.id
    )
    ...
    await add_task_labor_session(
        db, tenant_id, task.id, test_user.id, minutes_worked=180, actor_id=test_user.id
    )
