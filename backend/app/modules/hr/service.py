...
async def list_employees(tenant_id: UUID, db: AsyncSession) -> Sequence[Employee]:
    result = await db.execute(
        select(Employee).where(Employee.tenant_id == tenant_id).order_by(Employee.first_name, Employee.last_name)
    )
...
# Documents
# ------------------------------------------------------------------
async def list_employee_documents(tenant_id: UUID, employee_id: UUID, db: AsyncSession) -> Sequence[EmployeeDocument]:
...
async def generate_payroll(
    ...
    for emp in emps:
        ...
        advances_stmt = select(SalaryAdvance).where(
            ...
            # Normally, any approved advance not yet deducted is deducted. A future
            # policy may additionally restrict advances to the current period.
        )
    ...
    if slips:
        ...
        if total_gross > 0:
            ...
            if (total_inss_employee + total_inss_employer) > 0:
                items.append(
                    JournalItemCreate(
                        account_id=acc_449, debit=Decimal("0.00"), credit=(total_inss_employee + total_inss_employer)
                    )
                )
            if total_advances > 0:
...
async def list_payroll_slips(tenant_id: UUID, month: int, year: int, db: AsyncSession) -> Sequence[PayrollSlip]:
    result = await db.execute(
        select(PayrollSlip).where(
            PayrollSlip.tenant_id == tenant_id, PayrollSlip.period_month == month, PayrollSlip.period_year == year
        )
    )
