...
from app.modules.trips import known_routes_schemas as schemas
from app.modules.trips import known_routes_service as svc

router = APIRouter(prefix="/known-routes", tags=["known-routes"])


@router.get("", response_model=list[schemas.KnownRouteRead])
...
@router.post("", status_code=201, response_model=schemas.KnownRouteRead)
async def create_route(
    payload: schemas.KnownRouteCreate,
    principal: Annotated[Principal, Depends(require_permission(TRIPS_DISPATCH))],
    db: Annotated[AsyncSession, Depends(get_session)],
):
    return await svc.create_known_route(db, principal.tenant_id, payload.model_dump())


@router.patch("/{route_id}", response_model=schemas.KnownRouteRead)
async def update_route(
    route_id: UUID,
    payload: schemas.KnownRoutePatch,
    principal: Annotated[Principal, Depends(require_permission(TRIPS_DISPATCH))],
    db: Annotated[AsyncSession, Depends(get_session)],
):
    return await svc.update_known_route(
        db,
        principal.tenant_id,
        route_id,
...
@router.delete("/{route_id}", response_model=schemas.KnownRouteDeleteRead)
