...
| F7.4 | parcial | 179 testes de matriz: replay/conflito/escopo por tenant em 15 mutacoes com controlo negativo sem chave, conflito de payload de 2 para 17 das 54 operacoes, RBAC de 10 mutacoes x 6 papeis e 5 transicoes x 6 papeis derivadas de `ROLE_PERMISSIONS`, cobertura de permissoes declaradas vs rotas, e visibilidade salarial corrigida (salario e NIB deixam de ser legiveis por `viewer`/`manager`) | 8 permissoes ainda sem rota na Issue #45; 37 operacoes idempotentes por cobrir |
