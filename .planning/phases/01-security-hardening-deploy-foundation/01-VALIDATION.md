...
| alg=none rejection | 01 | 1 | SEC-05 | integration | `cd backend && python -m pytest tests/test_auth_api.py::test_alg_none_rejected -v` | ❌ W0 | ⬜ pending |
...
| Cross-tenant isolation | 01 | 2 | SEC-01..05 | integration | `cd backend && python -m pytest tests/test_cross_tenant.py -v` | ❌ W0 | ⬜ pending |
...
| Secure cookies (Next.js) | 01 | 2 | SEC-04 | manual | Review `apps/manager/app/api/auth/login/route.ts` contains `secure:` | ✅ after edit | ⬜ pending |
...
- [ ] `backend/tests/test_cross_tenant.py` — stubs for D-21 cross-tenant isolation tests
...
| Manager dashboard login sets Secure cookie in production | SEC-04 | Requires HTTPS deployment | Open browser devtools → Application → Cookies, verify Secure flag checked |
