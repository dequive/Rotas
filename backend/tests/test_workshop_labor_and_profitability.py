...
async def test_staff_hourly_rate_historical_effective_from_selection(db, tenant_id, test_user):
    """Seleciona a taxa histórica por completed_at e effective_from."""
...
async def test_multi_mechanic_labor_logging_and_void_exclusion(db, tenant_id, test_user):
    ...
    task = WorkOrderTask(
        tenant_id=tenant_id, work_order_id=wo["id"], description="Diagnóstico e Desmontagem", status="in_progress"
    )
    db.add(task)
    await db.flush()
...
async def test_post_billing_labor_immutability_guard_409(db, tenant_id, test_user):
    ...
    wo_payload = WorkOrderCreate(
        vehicle_id=vehicle.id, planned_work="Troca"
    )
    wo = await create_work_order(db, tenant_id, wo_payload, actor_id=test_user.id)
...
async def test_work_order_profitability_with_multi_quote_consolidated_invoice_fk_join(db, tenant_id, test_user):
    """Valida a FK BillingItem.work_order_id com orçamentos consolidados."""
    ...
    wo_res = await db.execute(
        select(WorkOrder).where(WorkOrder.tenant_id == tenant_id, WorkOrder.vehicle_id == vehicle.id)
    )
