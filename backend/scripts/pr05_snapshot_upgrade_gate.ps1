...
param(
    ...
    [Parameter(Mandatory = $true)]
    [ValidateSet("synthetic", "staging_anonymized", "production_anonymized")]
    [string]$SnapshotClass,

    [Parameter(Mandatory = $true)]
    [string]$AnonymizationEvidencePath,

    [Parameter(Mandatory = $true)]
    [string]$IntegritySqlPath,
...
if (-not $env:ENVIRONMENT) {
    throw "ENVIRONMENT must be supplied through the process environment."
}
if (-not $env:JWT_SECRET_KEY) {
    throw "JWT_SECRET_KEY must be supplied through the process environment."
}
foreach ($evidencePath in @($AnonymizationEvidencePath, $IntegritySqlPath)) {
    if (-not (Test-Path -LiteralPath $evidencePath -PathType Leaf)) {
        throw "Required evidence file not found: $evidencePath"
    }
}

$anonymizationEvidence = Get-Content -LiteralPath $AnonymizationEvidencePath -Raw |
    ConvertFrom-Json
if ($anonymizationEvidence.schema_version -ne 1) {
    throw "Anonymization evidence schema_version must be 1."
}
if ($anonymizationEvidence.source_database -ne $SourceDatabase) {
    throw "Anonymization evidence source_database does not match SourceDatabase."
}
if ($anonymizationEvidence.snapshot_class -ne $SnapshotClass) {
    throw "Anonymization evidence snapshot_class does not match SnapshotClass."
}
if ($anonymizationEvidence.contains_live_pii -ne $false) {
    throw "Anonymization evidence must assert contains_live_pii=false."
}
if ($anonymizationEvidence.anonymization_approved -ne $true) {
    throw "Anonymization evidence must assert anonymization_approved=true."
}
foreach ($requiredField in @("reviewed_by", "reviewed_at_utc", "approval_reference")) {
    if (-not $anonymizationEvidence.$requiredField) {
        throw "Anonymization evidence is missing $requiredField."
    }
}
try {
    [void][DateTimeOffset]::Parse($anonymizationEvidence.reviewed_at_utc)
}
catch {
    throw "Anonymization evidence reviewed_at_utc is invalid."
}
...
function Get-IntegrityFingerprint {
    ...
    $rows = & $psql `
        ...
        -f $IntegritySqlPath
    if ($LASTEXITCODE -ne 0) {
        throw "Integrity SQL failed for database $Database."
    }
    $normalized = (($rows | ForEach-Object { $_.TrimEnd() }) -join "`n").Trim()
    if (-not $normalized) {
        throw "Integrity SQL returned no rows for database $Database."
    }
...
try {
    ...
    $sourceIntegrity = Get-IntegrityFingerprint -Database $SourceDatabase
    ...
    $targetDatabaseUrl = $env:PR05_ALEMBIC_URL_TEMPLATE.Replace(
        "{database}",
        $TargetDatabase
    )
    $env:DATABASE_URL = $targetDatabaseUrl
    $env:ALEMBIC_DATABASE_URL = $targetDatabaseUrl
    ...
    $targetIntegrity = Get-IntegrityFingerprint -Database $TargetDatabase
    if ($targetIntegrity.hash -ne $sourceIntegrity.hash) {
        throw (
            "Integrity fingerprint changed after restore/upgrade: " +
            "$($sourceIntegrity.hash) -> $($targetIntegrity.hash)."
        )
    }
    ...
    $report = [ordered]@{
        ...
        anonymization_evidence_sha256 = (
            Get-FileHash -LiteralPath $AnonymizationEvidencePath -Algorithm SHA256
        ).Hash.ToLowerInvariant()
        anonymization_reviewed_by = $anonymizationEvidence.reviewed_by
        anonymization_approval_reference = $anonymizationEvidence.approval_reference
        integrity_sql_sha256 = (
            Get-FileHash -LiteralPath $IntegritySqlPath -Algorithm SHA256
        ).Hash.ToLowerInvariant()
        integrity_fingerprint_before = $sourceIntegrity.hash
        integrity_fingerprint_after = $targetIntegrity.hash
        integrity_result_lines = $targetIntegrity.line_count
