...
# As operações de RH usam permissões próprias e um principal autenticado;
# a autorização não depende de permissões de outros módulos.
...
@router.get(
    "/employees/{employee_id}/documents", response_model=list[schemas.EmployeeDocumentResponse]
)
...
@router.post(
    ...
    response_model=schemas.EmployeeDocumentResponse,
    status_code=201,
)
...
async def approve_salary_advance(
    ...
    stmt = select(SalaryAdvance).where(
        SalaryAdvance.tenant_id == principal.tenant_id, SalaryAdvance.id == advance_id
    )
...
async def export_ps2_file(
    ...
    return PlainTextResponse(
        content=csv_content,
        headers={"Content-Disposition": f"attachment; filename=vencimentos_{year}_{month:02d}.csv"},
    )
