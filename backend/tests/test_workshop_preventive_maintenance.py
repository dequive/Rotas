...
async def test_preventive_matching_and_automatic_next_cycle_creation_on_release(db, tenant_id, test_user):
    """Faz matching no fecho e gera o próximo ciclo pelo odómetro real."""
    ...
    await close_work_order(
        db, tenant_id, wo["id"], payload=WorkOrderCloseRequest(notes="Concluído com sucesso"), actor_id=test_user.id
    )
...
async def test_ownership_scope_all_inheritance(db, tenant_id, test_user):
    ...
    v_cust = Vehicle(
        tenant_id=tenant_id, plate="ALL-CUST", ownership_type="customer", customer_client_id=client.id, current_km=1000
    )
    db.add_all([v_fleet, v_cust])
    await db.flush()
