...
describe("contrato HTTP partilhado", () => {
  it("repete uma leitura em erro transitório e respeita Retry-After", async () => {
    const fetchMock = vi
      .fn<typeof fetch>()
  ...
  it("converte timeout num erro tipado e retryable", async () => {
  ...
  it("preserva código, mensagem e detalhes do envelope ROTAS", async () => {
    const error = await responseToHttpError(
      new Response(
        JSON.stringify({
          error: {
            code: "stock_conflict",
...
describe("Manager BFF", () => {
  it("atribui idempotência automática às mutações e usa a política comum", async () => {
    const fetchMock = vi
      .fn<typeof fetch>()
    ...
    expect(String(url)).toContain(
      encodeURIComponent("/api/v1/trips"),
    );
