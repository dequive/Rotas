...
class PurchaseOrderPDF(BaseReportEngine):
    ...
    def build_document(self):
        ...
        self.cell(
            0,
            6,
            getattr(self.supplier, "tax_id", "N/A") or "N/A",
            border=0,
            new_x="LMARGIN",
            new_y="NEXT",
        )
        ...
        self.cell(
            50, 10, " Valor Estimado", border=1, fill=True, align="R", new_x="LMARGIN", new_y="NEXT"
        )
        ...
        # O Valor estimado
        amount_str = (
            f"{float(self.po.estimated_amount):,.2f} {self.po.currency}"
            if self.po.estimated_amount
            else "Sob Consulta"
        )
        self.cell(
            50, end_y - start_y, amount_str, border=1, align="R", new_x="LMARGIN", new_y="NEXT"
        )
