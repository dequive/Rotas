...
| F7.4 | parcial | 199 testes de matriz: replay/conflito/escopo por tenant em 15 mutacoes HTTP e nos 9 tipos de entidade do sync offline, conflito de payload de 2 para 17 das 54 operacoes, RBAC de 10 mutacoes e 5 transicoes x 6 papeis derivadas de `ROLE_PERMISSIONS`, cobertura de permissoes declaradas vs rotas, visibilidade salarial corrigida, e isolamento de falha por operacao no lote de sincronizacao | 8 permissoes ainda sem rota na Issue #45; 37 operacoes idempotentes HTTP por cobrir |
