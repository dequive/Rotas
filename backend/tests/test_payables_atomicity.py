...
async def _seed_payable(
    db,
    tenant_id,
    *,
    include_bank_account: bool = True,
    canonical_account_codes: bool = False,
):
    supplier = ThirdParty(
        tenant_id=tenant_id,
    ...
    accounts = [
        Account(
            tenant_id=tenant_id,
            code="4.2" if canonical_account_codes else "42",
            name="Fornecedores",
    ...
    if include_bank_account:
        accounts.append(
            Account(
                tenant_id=tenant_id,
                code="1.2" if canonical_account_codes else "12",
...
async def test_supplier_payment_accepts_seeded_canonical_pgc_codes(
    async_client, auth_headers, db, tenant_id
):
    invoice = await _seed_payable(db, tenant_id, canonical_account_codes=True)
    response = await async_client.post(
        f"/api/v1/payables/invoices/{invoice.id}/pay",
