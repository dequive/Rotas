...
class Employee(Base):
    ...
    __table_args__ = (
        UniqueConstraint("tenant_id", "nif_nuit", name="uq_employees_tenant_nif"),
    )
    ...
    driver_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("drivers.id", ondelete="SET NULL"), index=True, nullable=True)
    ...
    role: Mapped[str] = mapped_column(String(80), nullable=False) # e.g. "driver", "mechanic", "manager"
    ...
    status: Mapped[str] = mapped_column(String(30), default="active", index=True) # active, on_leave, terminated
...
class EmployeeDocument(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    tenant_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("tenants.id"), index=True, nullable=False)
    employee_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("employees.id", ondelete="CASCADE"), index=True, nullable=False)
    
    document_type: Mapped[str] = mapped_column(String(80), nullable=False) # "id_card", "passport", "contract", "health_card"
    ...
    file_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), nullable=True) # link to s3 file
...
class Absence(Base):
    ...
    absence_type: Mapped[str] = mapped_column(String(40), nullable=False) # "vacation", "sick_leave", "unjustified"
...
class PayrollSlip(Base):
    ...
    __table_args__ = (
        UniqueConstraint("tenant_id", "employee_id", "period_month", "period_year", name="uq_payroll_slips_tenant_emp_period"),
    )
    ...
    gross_salary: Mapped[Decimal] = mapped_column(Numeric(14, 2), nullable=False, default=0) # Iliquido
    ...
    net_salary: Mapped[Decimal] = mapped_column(Numeric(14, 2), nullable=False, default=0) # Liquido a Receber
    
    status: Mapped[str] = mapped_column(String(30), default="draft", index=True) # draft, approved, paid
...
class PayrollSlipLine(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    tenant_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("tenants.id"), index=True, nullable=False)
    payroll_slip_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("payroll_slips.id", ondelete="CASCADE"), index=True, nullable=False)
    ...
    amount: Mapped[Decimal] = mapped_column(Numeric(14, 2), nullable=False) # Can be negative for deductions
...
class SalaryAdvance(Base):
    ...
    status: Mapped[str] = mapped_column(String(30), default="pending", index=True) # pending, approved, rejected, deducted
