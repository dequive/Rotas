...
async def get_employee(tenant_id: UUID, employee_id: UUID, db: AsyncSession) -> Employee:
    ...
    if not emp or emp.tenant_id != tenant_id:
        raise ApiError("EMPLOYEE_NOT_FOUND", status.HTTP_404_NOT_FOUND, "Colaborador não encontrado.")
    return emp


async def create_employee(
    tenant_id: UUID, payload: EmployeeCreate, db: AsyncSession
) -> Employee:
    if payload.nif_nuit:
        stmt = select(Employee).where(Employee.tenant_id == tenant_id, Employee.nif_nuit == payload.nif_nuit)
        if await db.scalar(stmt):
            raise ApiError("NUIT_EXISTS", status.HTTP_400_BAD_REQUEST, "Já existe um colaborador com este NUIT.")
...
async def generate_payroll(
    ...
    if payload.employee_id:
        ...
        result = await db.execute(select(Employee).where(Employee.tenant_id == tenant_id, Employee.status == "active"))
    ...
    for emp in emps:
        ...
        if worked_days < 0: worked_days = Decimal("0")
        ...
        if emp_vars:
            if emp_vars.overtime_hours > 0:
                ...
                r02 = PayrollSlipLine(
                    tenant_id=tenant_id, code="R02", description="HORAS EXTRAORDINÁRIAS",
                    quantity=emp_vars.overtime_hours, unit_price=overtime_rate, amount=overtime_amount,
                    irps_tax_percentage=emp.irps_tax_percentage, is_taxable_inss=True, is_taxable_syndicate=False
                )
                lines.append(r02)
            ...
            if emp_vars.meal_allowance_days > 0:
                ...
                r03 = PayrollSlipLine(
                    tenant_id=tenant_id, code="R03", description="SUBSÍDIO DE ALIMENTAÇÃO",
                    quantity=emp_vars.meal_allowance_days, unit_price=meal_allowance_rate, amount=meal_amount,
                    irps_tax_percentage=emp.irps_tax_percentage, is_taxable_inss=True, is_taxable_syndicate=False
                )
                lines.append(r03)
            ...
            if emp_vars.other_allowances > 0:
                ...
                r04 = PayrollSlipLine(
                    tenant_id=tenant_id, code="R04", description="OUTROS SUBSÍDIOS",
                    quantity=Decimal("1.00"), unit_price=emp_vars.other_allowances, amount=emp_vars.other_allowances,
                    irps_tax_percentage=emp.irps_tax_percentage, is_taxable_inss=True, is_taxable_syndicate=False
                )
                lines.append(r04)
            ...
            if emp_vars.bonus > 0:
                ...
                r05 = PayrollSlipLine(
                    tenant_id=tenant_id, code="R05", description="BÓNUS E PRÉMIOS",
                    quantity=Decimal("1.00"), unit_price=emp_vars.bonus, amount=emp_vars.bonus,
                    irps_tax_percentage=emp.irps_tax_percentage, is_taxable_inss=True, is_taxable_syndicate=False
                )
                lines.append(r05)
        ...
        irps_deduction = _decimal(total_taxable * (emp.irps_tax_percentage / Decimal("100"))) if emp.irps_tax_percentage > 0 else Decimal("0.00")
        if irps_deduction > 0:
            d02 = PayrollSlipLine(
                tenant_id=tenant_id,
                code="D02",
        ...
        advances_stmt = select(SalaryAdvance).where(
            ...
            # Optional: check if the advance date is in this month, but normally any approved advance not yet deducted is deducted.
        )
    ...
    if slips:
        ...
        if total_gross > 0:
            ...
            async def get_account(code: str, name: str, act_type: str) -> str:
                res = await db.execute(select(Account).where(Account.tenant_id == tenant_id, Account.code == code))
...
async def list_payroll_slips(tenant_id: UUID, month: int, year: int, db: AsyncSession) -> Sequence[PayrollSlip]:
    result = await db.execute(
        select(PayrollSlip)
        .where(
            PayrollSlip.tenant_id == tenant_id,
    ...
    for slip in slips:
        lines_res = await db.execute(select(PayrollSlipLine).where(PayrollSlipLine.payroll_slip_id == slip.id))
