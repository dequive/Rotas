...
export async function responseToHttpError(response: Response): Promise<HttpContractError> {
  let body: HttpErrorBody = {};
  try {
    body = (await response.clone().json()) as HttpErrorBody;
  } catch {
    // Empty and non-JSON responses still receive a stable typed error.
  }
  ...
  return new HttpContractError(
    ...
    code,
    details,
    DEFAULT_RETRY_STATUSES.includes(response.status as (typeof DEFAULT_RETRY_STATUSES)[number]),
  );
}
...
function delayForAttempt(attempt: number, baseDelayMs: number, maxDelayMs: number, random: () => number): number {
...
export async function requestWithPolicy(
  ...
  const sleep = policy.sleep ?? ((delayMs: number) => new Promise<void>((resolve) => globalThis.setTimeout(resolve, delayMs)));
  ...
  for (let attempt = 0; ; attempt += 1) {
    ...
    try {
      ...
      if (canRetry && attempt < maxRetries && retryStatuses.has(response.status)) {
        const retryDelay = Math.min(
          maxDelayMs,
          retryAfterMs(response) ?? delayForAttempt(attempt, baseDelayMs, maxDelayMs, random),
        );
      ...
      if (init.signal?.aborted) {
        throw new HttpContractError("Pedido cancelado.", 0, "request_aborted", undefined, false);
      }
      const retryable = timedOut || error instanceof TypeError;
      if (canRetry && retryable && attempt < maxRetries) {
        await sleep(delayForAttempt(attempt, baseDelayMs, maxDelayMs, random));
        continue;
      }
